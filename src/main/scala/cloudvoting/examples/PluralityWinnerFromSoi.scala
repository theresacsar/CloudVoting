...
import cloudvoting.utils.importpreflib.readSoi
import org.apache.log4j.{Level, Logger}
import org.apache.spark.{SparkConf, SparkContext}

object PluralityWinnerFromSoi {

  /**
    * Calculates the Plurality Winner from a SOI file. Only use PrefLib format as input!
    *
  ...
  def main(args: Array[String]): Unit = {
    if(args.length == 0){
      ...
      conf.setAppName("CloudVoting Example: PluralityWinnerFromSoi")
      val sc = new SparkContext(conf)

      println("CloudVoting: Reading File from " + args(0))

      val graph = readSoi(args(0), sc)
