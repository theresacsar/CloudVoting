...
object PluralityWinnerFromSoc {

  /**
    * Calculates the Plurality Winner from a SOC file. Only use PrefLib format as input!
    *
  ...
  def main(args: Array[String]): Unit = {
    if(args.length == 0){
      ...
      conf.setAppName("CloudVoting Example: PluralityWinnerFromSoc")
